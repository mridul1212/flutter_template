...
enum AppDestination { splash, onboarding, login, profileCompletion, home }
...
class AppRouterCubit extends Cubit<AppRouterState> {
    ...
    if (valid) {
      ...
      if (user != null) {
        if (!user.isProfileComplete) {
          emit(state.copyWith(destination: AppDestination.profileCompletion, user: user));
        } else {
  ...
  void onProfileCompleted(UserEntity user) {
    emit(state.copyWith(destination: AppDestination.home, user: user));
  }
